...
from lifeguard.views.vpool.elasticity_planning import plan_expansion, plan_shrink, plan_update
from lifeguard.views.vpool.elasticity_tasks import get_runnable_from_action_id
from lifeguard.jira_api import JiraApi
from lifeguard.tasks import all_pools_and_members
...
def run():
  ...
  implement = True
  try:
    opts, args = getopt.getopt(sys.argv[1:], "h:c", ["cowboy-mode=", "help=", "plan=", "implement="])
    for opt, arg in opts:
      if opt in ['-h', '--help']:
        usage()
        sys.exit()
      elif opt == "--cowboy-mode":
